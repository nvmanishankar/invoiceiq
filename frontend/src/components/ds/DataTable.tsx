...
export function DataTable<T>({ columns, rows, rowKey, rowClassName, caption }: {
  columns: Column<T>[]
  rows: T[]
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-collapse text-[14px]">
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={rowKey(r, i)} className={cn("border-b border-line last:border-b-0 hover:bg-hover/70", rowClassName?.(r))}>
              {columns.map((c) => (
